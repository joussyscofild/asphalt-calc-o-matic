...
const BlogPost = ({ id, title, excerpt, author, date, category, tags, imageUrl, featured }: BlogPostProps) => {
  return (
    <article className={`bg-white rounded-xl overflow-hidden shadow-sm transition-all duration-300 hover:shadow-md ${featured ? 'border-l-4 border-safety' : ''}`}>
      {imageUrl && (
      ...
      <div className="p-6">
        <div className="flex justify-between items-center mb-3">
          <Link 
            ...
            className="text-xs font-semibold uppercase text-concrete hover:text-asphalt transition-colors"
          >
            {category}
          </Link>
        ...
        <Link to={`/blog/${id}`}>
          <h3 className="text-xl font-bold text-asphalt mb-3 hover:text-safety-dark transition-colors">
            {title}
          </h3>
        </Link>
        
        <p className="text-concrete-dark mb-4">{excerpt}</p>
